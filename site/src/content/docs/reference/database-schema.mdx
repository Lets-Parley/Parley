...
<SpecTable rows={[
  ...
  { name: "0036_session_tokens_sweep_idx", value: "session_tokens (last_used_at), (created_at), (expires_at) where expires_at is not null", note: "Btree indexes for the hourly session-token sweep. Regular create index — migrate.go wraps each file in a transaction, so concurrently is unavailable. last_used_at is rewritten on every session touch, so this index prevents HOT updates of that column; created_at and expires_at are immutable after insert. Pins the minimum image version: migrations are forward-only, and an older image refuses to boot against a newer schema" },
  { name: "0037_notification_sounds", value: "users.notification_sounds, sessions.poker_round_version", note: "Both use constant defaults, so existing identities remain muted and existing poker sessions begin at round version 0" },
  { name: "0038_standup_schedules", value: "standup_schedules, standup_schedule_slots", note: "Scheduled async standups. New tables, so no backfill. Pins the minimum image version: migrations are forward-only, and an older image refuses to boot against a newer schema" },
  { name: "0039_standup_webhooks", value: "standup_webhooks, standup_webhook_deliveries", note: "Standup webhooks and their delivery outbox. New tables, so no backfill. Pins the minimum image version, as above" },
  { name: "0040_ics_feed_tokens", value: "ics_feed_tokens", note: "One personal calendar token per user. The token is stored as a sha256 digest plus an 8-character lookup prefix. New table, so no backfill. Pins the minimum image version, as above" },
  { name: "0041_standup_follow_through", value: "standup_commitments.closed_reason, standup_mentions", note: "closed_reason is landed or dropped. Every row already closed is backfilled to landed, the only close that existed before; open rows stay null. standup_mentions is a new table, so no backfill. Append-only: rolling back means restoring from backup. Pins the minimum image version, as above" },
  { name: "0042_standup_away", value: "standup_away, standup_trend_days", note: "Self-set away days for async standups, and each scheduled standup's frozen trend counts. New tables, so no backfill; a past scheduled day is frozen lazily on the first trend read. Pins the minimum image version, as above" },
  { name: "0043_embed_handoffs", value: "embed_handoffs, session_tokens.embedded", note: "The meeting-client sign-in handoff and the marker on the session it mints. The new column defaults to false, so every existing session stays an ordinary one. Pins the minimum image version, as above" },
]} />
...
| `notification_sounds` | boolean | Per-user preference, false by default. Returned only with the caller's identity |
...
| `created_at`, `last_used_at` | timestamptz | `last_used_at` drives the `SESSION_IDLE_TTL` idle window, refreshed by any write or WebSocket connect and never by a `GET`; `created_at` drives the `SESSION_MAX_TTL` absolute cap, which nothing extends |
| `expires_at` | timestamptz | Null for ordinary sessions. A redeemed guest link sets it; the token dies at that instant regardless of idle activity |
| `embedded` | boolean | True for a session minted for a meeting client's frame (12 hours from minting, participant power only). False for everything else |
...
| `poker_round_version` | bigint | Increments when a story is selected or the selected story is reset, including an empty round |
...
| `seen_at` | timestamptz | Nullable, no default. When the recipient read it; null is unread. Shown only to the recipient, never aggregated, never exported. Rows that existed before 0044 were backfilled as read |
...
`standup_schedules` holds at most one schedule per space (`space_id` unique, on
delete cascade): `weekdays` (a `smallint[]` of `0`–`6`, Sunday first),
`open_time`, `timezone` (IANA), `window_minutes` (1–1440), `enabled`, and
`updated_by`. `updated_by` is nullable and `references users(id) on delete set
null`. It records the user who last saved the schedule. Deleting that user
...
`standup_webhooks` holds at most one webhook per space (`space_id` primary key,
...
One personal standup calendar per user (`user_id` primary key, `on delete
cascade`). `token_hash` is the sha256 of the token's raw bytes, the same shape
...
One meeting-client sign-in attempt, keyed by `challenge_hash` — the 32-byte
...
One member asking another for help with a blocker, in one standup:
...
The rows never reach the room's shared state. They are read one person at a
time: the people who asked you, and the people you asked.

### standup_away

A person's self-set away days: `id`, `user_id` (on delete cascade), and an
...
One frozen trend day per scheduled async standup: `id`, `session_id` (unique,
...
| `plugin_jobs` | Deferred work. The claim still keys off `run_at`. A five-field cron on `parley_job_enqueue` is converted to the next `run_at` at schedule time; invalid expressions are refused rather than stored |
