...
Forty migrations, embedded in the binary and applied at boot. Postgres only —
...
<SpecTable rows={[
  ...
  { name: "0042_standup_away", value: "standup_away", note: "Self-set away days for async standups. New table, so no backfill. Pins the minimum image version, as above" },
]} />
...
by id, so the digest does not read them as owing an answer; an ended one lists
nobody. The team trend leaves them out of the eligible count. Nothing reads
these rows into a per-person participation figure, and no table stores one:
the trend is computed on request from `sessions`, `members`, `standup_entries`
and this table, and returns team ratios only.
