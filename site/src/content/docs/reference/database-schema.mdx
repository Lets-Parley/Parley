...
Forty-two migrations, embedded in the binary and applied at boot. Postgres only —
...
<SpecTable rows={[
  ...
  { name: "0042_embed_handoffs", value: "embed_handoffs, session_tokens.embedded", note: "The meeting-client sign-in handoff and the marker on the session it mints. The new column defaults to false, so every existing session stays an ordinary one. Pins the minimum image version, as above" },
]} />
