...
find it, and renaming or deleting any session in it. All six routes go through
the same `requireSpaceOwner` middleware, which resolves the slug, reads the caller's
