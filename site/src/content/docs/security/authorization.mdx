...
a round; toggle poker auto-reveal; save an estimate; start or advance a standup;
close or reopen the session; or hand the role to someone else.
...
<Capability rows={[
  ...
  { name: "Undo, or an audit trail, for an owner deletion", status: "not-built", detail: "An owner can delete a space or a session, and it is gone — no soft delete, no archive. A space delete emits a stdout <code>security event</code> line; a session delete does not, and neither writes <code>org_audit_log</code> (that table covers org-admin custody actions only, and is write-only — <a href='https://github.com/Lets-Parley/Parley/issues/399'>#399</a>). Restore from a <a href='/operations/backups-and-recovery/'>backup</a> if you need one back." },
