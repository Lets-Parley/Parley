...
<VerifiedStamp version="unreleased" source={["internal/auth/oidc.go", "internal/api/auth.go", "internal/api/principal.go", "internal/api/me.go", "internal/store/users.go", "internal/hub/hub.go", "internal/api/ics.go", "internal/standup/ics.go", "internal/api/embed.go", "internal/store/embed.go"]} />
...
1. The frame makes a random RFC 7636 verifier (43–128 characters) and sends
   ...
   display code. A frame should do this when its panel loads, not on click: a
   popup opened after an `await` is blocked, so the Sign in click must do
   nothing but open `/embed/signin?c=<challenge>`. That URL and the display
   code can also be shown for opening by hand.
2. `/embed/signin` is an ordinary top-level Parley page (`X-Frame-Options:
   ...
   mode, names the meeting client only from an enabled provider, shows the
   display code, and binds the handoff only when the person presses Continue.
   Its form post is covered by the cross-site check and the `Lax` cookie. Link
   guests and embedded sessions cannot bind.
...
a bearer that is not an embedded token resolves to nobody.

An embedded session has participant power only. Besides everything the same
person is refused with a cookie, it is refused org administration (custody,
plugin administration, the org purge), minting signed links, redeeming one,
renaming (`POST /api/me`) and minting a calendar feed. Signing out
(`DELETE /api/me`) deletes the embedded token.
