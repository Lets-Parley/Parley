...
<VerifiedStamp version="unreleased" source={["internal/auth/oidc.go", "internal/api/auth.go", "internal/api/principal.go", "internal/api/me.go", "internal/store/users.go", "internal/hub/hub.go", "internal/api/ics.go", "internal/standup/ics.go"]} />
...
one. Revoking it from the profile, revoking
the person's org membership, or deleting the user all make the URL stop
resolving. Org-membership revoke and the feed revocation are one transaction.
User delete removes the row by foreign key.
