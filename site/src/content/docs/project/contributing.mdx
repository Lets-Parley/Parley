...
1. **Write the package and register it.** Give it a `Kind()` constructor
   ...
   client-safe data, `NewConfig`, an optional `CSV`, and its `Actions` dispatch
   table. Add it to the slice in `internal/api/router.go`:
   ...
   action the same thing. Any action that must not run on a closed session
   checks that itself — the ended-session guard is not in shared middleware.
