...
- **That database is Parley's alone.** Sharing a Postgres *server* with other
  ...
  *blocking*, so a second Parley — or any application reusing the same id — in
  ...
  never becomes ready.
- **A verifying `sslmode` on `DATABASE_URL`.** Parley refuses to start if
  `sslmode` is absent, `disable`, `allow` or `prefer` unless
  `DATABASE_ALLOW_PLAINTEXT=true` — those modes can talk to Postgres in the clear.
  Use `?sslmode=verify-full&sslrootcert=/etc/parley/ca/ca.pem`. That
  path is inside the container; mount the CA there with the ConfigMap in
  [Air-gapped](/operations/air-gapped/#kubernetes). `require` still boots
  (it encrypts but never checks the server's certificate); `verify-ca` checks
  the CA but not the hostname. A URI the server cannot complete a TLS
  handshake with still retries six times with doubling backoff — about a
  minute of `postgres not reachable yet, retrying` — before
...
- **A quick sanity check** from anywhere that can reach the database (set
  ...
  # sslrootcert is the CA file on the machine running psql, not a container path
  psql 'postgres://parley@host:5432/parley?sslmode=verify-full&sslrootcert=./ca.pem' \
    -c "select has_database_privilege('parley', 'parley', 'CREATE')" \
    -c 'select version()'
  ```

  A `t` result confirms the role can install `pgcrypto`. If that succeeds,
  Parley will migrate. The URI you store in the secret uses the in-pod path
  `/etc/parley/ca/ca.pem` after the ConfigMap mount below.
...
kubectl create secret generic parley \
  --from-literal=database-url='postgres://parley:secret@host:5432/parley?sslmode=verify-full&sslrootcert=/etc/parley/ca/ca.pem'
...
`sslrootcert=/etc/parley/ca/ca.pem` is inside the container. The chart does
not mount a CA by default — add the ConfigMap from
[Air-gapped](/operations/air-gapped/#kubernetes). `require` still boots if
...
- **`image.tag`** — empty tracks the chart's `appVersion`, which always names a
  published release. Pin it only to move off that. The FIPS image is the same
  chart with `--set image.tag=<version>-fips`; `latest-fips` is refused the
  same way `latest` is. See [Cryptography](/security/cryptography/).
- **`image.digest`** — empty keeps `repository:tag`. Set it to `sha256:…` to
  render `repository@digest` and ignore the tag, the same way
  `tests.image.digest` pins the `helm test` pod. See
  [Air-gapped installs](/operations/air-gapped/).
...
- **`networkPolicy.egress.enabled`** — off by default, ingress-only being the
  norm. For a cluster that requires default-deny egress, this adds egress
  rules for exactly the three peers Parley uses: DNS, Postgres, and the IdP.
  See [below](#egress-is-off-by-default-too-for-the-same-reason).
- **`metrics.enabled`** — off by default. `METRICS_ENABLED=true` serves
  `/metrics` on the app port, unauthenticated. The Ingress path is `/` Prefix,
  so leaving it routed publishes the exposition. Admit the scraper's pods with
  `networkPolicy.extraIngress` and keep the path off the public hostname; see
...
Most of the [configuration reference](/reference/configuration/) is reachable
...
`valueFrom` works too — each entry is a full EnvVar, not a string map. The
variables themselves, and what each limit counts, live in the
[configuration reference](/reference/configuration/); this page does not
re-list them.
...
example; see [Air-gapped installs](/operations/air-gapped/#trusting-a-private-ca).
...
[below](#it-refuses-to-render-a-deployment-that-cannot-work).
...
- **A moving image tag** — `latest`, `latest-fips`, `LATEST`, `Main`, or one with trailing
...
- **A malformed `extraEnv`** — a map instead of a list, or an entry with no
  `name`. See [above](#setting-variables-the-chart-has-no-knob-for).
- **`networkPolicy.egress.enabled` with no `networkPolicy.egress.postgres.cidr`**
  — see [above](#egress-is-off-by-default-too-for-the-same-reason).
...
`sslrootcert=/etc/parley/ca/ca.pem` is a path inside the container. The
plain manifest mounts no CA; add the ConfigMap volume from
[Air-gapped](/operations/air-gapped/#kubernetes) or the handshake fails.
...
So after any upgrade of a multi-replica install, check where the pods actually
are. This is the real safety net, not a formality:

```sh
kubectl --namespace parley get pods -o wide
```

Two different `NODE` values is the answer you want. If they have collapsed onto
...
**If you are replacing a hand-rolled policy with this one, add it before you
...
The NetworkPolicy above is ingress-only. Parley makes exactly three kinds of
...
helm upgrade parley oci://ghcr.io/lets-parley/charts/parley --reuse-values \
    ...
    --set networkPolicy.egress.idp.port=443
```

DNS to `kube-system` (UDP and TCP, port 53) is always included once egress is
...
Off by default for the same two reasons as the ingress half: a CNI that
ignores NetworkPolicies would give a false sense of safety, and CI already
asserts the render and the refusal above — see
[below](#it-refuses-to-render-a-deployment-that-cannot-work).
