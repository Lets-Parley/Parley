...
- **A verifying `sslmode` on `DATABASE_URL`.** Parley refuses to start if
  ...
  `DATABASE_ALLOW_PLAINTEXT=true` — those modes can talk to Postgres in the
  clear. Use `?sslmode=verify-full&sslrootcert=/path/to/ca.pem`. `require`
  encrypts but never checks the server's certificate; `verify-ca` checks the
  CA but not the hostname. A URI the server cannot complete a TLS handshake
  with still retries six times with doubling backoff — about a minute of
...
- **A quick sanity check** from anywhere that can reach the database (set
  ...
  psql 'postgres://parley@host:5432/parley?sslmode=verify-full&sslrootcert=/path/to/ca.pem' \
...
kubectl create secret generic parley \
  --from-literal=database-url='postgres://parley:secret@host:5432/parley?sslmode=verify-full&sslrootcert=/path/to/ca.pem'
