...
`DATABASE_URL`, `BASE_URL`, `LOG_LEVEL`, `AUTH_MODE`, `POD_NAME`, the `OIDC_*`
set, and the proxy-trust pair (`TRUST_PROXY_HEADERS` /
`TRUSTED_PROXY_CIDRS`). Prefer the named values for those. Setting them here
