...
// a day. %s is the filter; s is the session, de.day_end the local midnight.
const freezeTrendDays = `
	...
	cross join lateral (
	    select ((sl.slot_date + 1)::timestamp at time zone sc.timezone) as day_end) de
...
// are ignored.
func FreezeEndedTrendDays(ctx context.Context, q execer, sessionIDs []string) error {
	if len(sessionIDs) == 0 {
		return nil
	}
	if _, err := q.Exec(ctx, fmt.Sprintf(freezeTrendDays, "s.id = any($1::uuid[]) and s.ended_at is not null"),
		sessionIDs); err != nil {
...
func FreezePastTrendDays(ctx context.Context, q execer, spaceID string, now time.Time) error {
	if _, err := q.Exec(ctx, fmt.Sprintf(freezeTrendDays,
		"s.space_id = $1 and (s.ended_at is not null or de.day_end <= $2)"), spaceID, now); err != nil {
