...
func (s *Spaces) Create(ctx context.Context, name, slug string) (Space, error) {
	var sp Space
	err := s.Pool.QueryRow(ctx,
		"insert into spaces (slug, name) values ($1, $2) returning id, slug, name",
		slug, name,
	).Scan(&sp.ID, &sp.Slug, &sp.Name)
...
func (s *Spaces) BySlug(ctx context.Context, slug string) (Space, error) {
	var sp Space
	err := s.Pool.QueryRow(ctx,
		"select id, slug, name from spaces where slug = $1", slug,
	).Scan(&sp.ID, &sp.Slug, &sp.Name)
