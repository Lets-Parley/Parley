 ...
 *
 * A native radio group, not a custom grid: roving tabindex, arrow keys and the
 * selected-state announcement all come from the platform, and the fieldset
 * legend names the group once.
 *
...
export function AvatarDialog({ me, onClose }: { me: Me; onClose: () => void }) {
  ...
  async function save() {
    ...
    try {
      await api("PATCH", "/api/me/avatar", { icon: picked, accessory: "" });
          ...
          {["", ...avatarIconIds].map((id) => (
            ...
            >
              <input
                ...
                className="sr-only"
              />
              <Avatar name={me.name} hue={me.avatarHue} icon={id} size="md" decorative />
