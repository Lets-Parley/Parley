...
Pin a version rather than `latest` — 0.9.1 is the current release. 0.2.2 is the
...
helm install parley oci://ghcr.io/lets-parley/charts/parley --version 0.9.1 \
...
time, so pass `--version 0.9.1` when you scale up. Above one replica the
...
```sh
helm upgrade parley oci://ghcr.io/lets-parley/charts/parley --version 0.9.1 \
  --reuse-values
