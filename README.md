...
- **Revoke means gone.** Revoking a link closes the sockets of everyone who
  ...
  part in. Guests vote fully in a poker room, and in a standup take a place in
  the round and get the turn when it comes round to them; starting, advancing
  and skipping the round stay with the facilitator.
...
docker run -d --name parley -p 8080:8080 \
  ...
  ghcr.io/lets-parley/parley:0.12.0
```

Pin a version rather than `latest` — 0.12.0 is the current release. 0.2.2 is the
...
helm install parley oci://ghcr.io/lets-parley/charts/parley --version 0.12.0 \
...
time, so pass `--version 0.12.0` when you scale up. Above one replica the
...
```sh
helm upgrade parley oci://ghcr.io/lets-parley/charts/parley --version 0.12.0 \
  --reuse-values
